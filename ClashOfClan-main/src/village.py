...
class Village:
    ...
    def generate_map(self):
        ...
        for wall in self.walls_top:                                     # wall is 1x1
            wall_obj = bd.Wall(wall, self, self.level)
    ...
    def remove_wall(self, wall_obj):
        ...
        if(wall_obj.level >= 3):
            ground_troops = barbarians+archers+stealth_archers
            for troop in ground_troops:
                dist = abs(troop.position[0]-wall_obj.position[0]) + abs(troop.position[1]-wall_obj.position[1])
                if(dist <= wall_obj.destruction_radius):
                    troop.deal_damage(wall_obj.destruction)
