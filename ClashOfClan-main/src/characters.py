...
def spawnStealth_Archer(pos):
    ...
    # convert tuple to list
    pos = list(pos)
    stealth_archer = Stealth(pos)
    stealth_archer.current = time.time()
...
def move_healer(V,King):
    ...
    for healer in healers:
        if(healer.alive == False):
            continue
        closest_troop = closest(healer.position, King)
        if(closest_troop == None):
            continue
        healer.target = closest_troop
        healer.move(closest_troop.position, King)

def closest(pos,King):
    closest_troop = King
    min = abs(King.position[0]-pos[0]) + abs(King.position[1] - pos[1])
    troops = barbarians + archers + stealth_archers+ dragons + balloons
    for troop in troops:
        manhattan = abs(troop.position[0]-pos[0]) + abs(troop.position[1] - pos[1])
        if(manhattan <= min):
            min = manhattan
            closest_troop = troop
...
class Healer:
    ...
    def move(self, pos, King):
        ...
        if(r+c < self.heal_range):
            self.healing(self.target, King)
            return
        elif(r == 0):
            if(pos[1] > self.position[1]):
                for i in range(self.speed):
    ...
    def healing(self, target, King):
        if(self.alive == False):
            return
        target.health = target.health+self.strength
        if target.health > target.max_health:
            target.health = target.max_health
        if(abs(King.position[0]-target.position[0]) + abs(King.position[1] - target.position[1]) <= self.heal_radius):
                King.health = min(King.health+self.strength,King.max_health)

        troops = barbarians + archers + stealth_archers + dragons + balloons
        for troop in troops:
            if(abs(troop.position[0]-target.position[0]) + abs(troop.position[1] - target.position[1]) <= self.heal_radius):
                troop.health = min(troop.health+self.strength,troop.max_health)
        if(abs(King.position[0]-target.position[0]) + abs(King.position[1] - target.position[1]) <= self.heal_radius):
                King.health = min(King.health+self.strength,King.max_health)
        if target.health > target.max_health:
            target.health = target.max_health
